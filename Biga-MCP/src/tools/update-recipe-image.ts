...
import { validateUpdateRecipeImage, type UpdateRecipeImageInput } from './validation.js';
...
import type { ImageSource, ImagesJsonb } from '../types/images.js';
...
export async function updateRecipeImage(
  ...
  const alt = input.alt ?? generateAltText({
    title: (existing['title'] as string) ?? input.slug.replace(/-/g, ' '),
    ingredients: (existing['ingredients'] as Array<{ rawString: string; entity?: { name: string } | null }>) ?? [],
  });
  ...
  let imageEntry;
  try {
    imageEntry = await uploadImageToStorage(
      input.slug,
      input.role,
      buffer,
      {
  ...
  const currentImages = (existing['images'] as ImagesJsonb) ?? {};
  const updatedImages = { ...currentImages, [input.role]: imageEntry };

  await sql`
    UPDATE recipes
    SET images = ${sql.json(updatedImages as never)}, updated_at = now()
