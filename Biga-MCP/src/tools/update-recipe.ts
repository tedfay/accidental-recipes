...
import { validateUpdateRecipe, type UpdateRecipeInput } from './validation.js';
...
export async function updateRecipe(
  ...
  if (updates.includes('meta') && updates.includes('headnote') && updates.includes('ingredients') && updates.includes('steps')) {
    const metaVal = values[updates.indexOf('meta')] as string;
    const headnoteVal = values[updates.indexOf('headnote')] as string | null;
    const ingredientsVal = values[updates.indexOf('ingredients')] as string;
    const stepsVal = values[updates.indexOf('steps')] as string;
    await sql`
      UPDATE recipes SET
        meta = ${metaVal}::jsonb,
    ...
    for (let i = 0; i < updates.length; i++) {
      ...
      switch (field) {
        case 'meta':
          await sql`UPDATE recipes SET meta = ${value as string}::jsonb WHERE slug = ${input.slug}`;
          break;
        case 'headnote':
          await sql`UPDATE recipes SET headnote = ${value as string | null} WHERE slug = ${input.slug}`;
          break;
        case 'ingredients':
          await sql`UPDATE recipes SET ingredients = ${value as string}::jsonb WHERE slug = ${input.slug}`;
          break;
        case 'steps':
          await sql`UPDATE recipes SET steps = ${value as string}::jsonb WHERE slug = ${input.slug}`;
          break;
        case 'derived_from_recipe_id':
          if (value) {
            await sql`UPDATE recipes SET derived_from_recipe_id = ${value as string}::uuid WHERE slug = ${input.slug}`;
          } else {
