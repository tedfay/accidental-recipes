 ...
 */
function validateApiKey(key: string | undefined): PermissionLevel | null {
  if (!key) return null;
  const readKey = process.env['MCP_API_KEY_READ'];
  const writeKey = process.env['MCP_API_KEY_WRITE'];
  if (key === writeKey) return 'write';
  if (key === readKey) return 'read';
  return null;
}
...
function createConfiguredServer(): McpServer {
  ...
  server.tool(
    'create_recipe',
    'Create a new recipe record. Ingredients are created without entity resolution — enrichment runs separately.',
    {
  ...
  server.tool(
    'update_recipe',
    'Update an existing recipe by slug. Partial updates — only provided fields are changed. Content changes auto-increment version.',
    {
  ...
  server.tool(
    'publish_recipe',
    'Change recipe status from draft to live. Separate tool to keep the publish action explicit.',
    {
  ...
  server.tool(
    'update_recipe_image',
    'Download an image from a URL, optionally embed credit/copyright metadata (EXIF, JPEG only), upload to Supabase Storage, and merge the entry into the recipe images JSONB field. Stores storage_path; public URLs are composed at read time.',
    {
      slug: z.string().describe('Recipe slug'),
    ...
    if (req.method === 'POST' && req.url === '/tools/call') {
      try {
        ...
        const toolMap: Record<string, ToolHandler> = {
          ...
          create_recipe: (a) => createRecipe(a as unknown as Parameters<typeof createRecipe>[0]),
          update_recipe: (a) => updateRecipe(a as unknown as Parameters<typeof updateRecipe>[0]),
          publish_recipe: (a) => publishRecipe(a.slug as string),
          update_recipe_image: (a) => updateRecipeImage(a as unknown as Parameters<typeof updateRecipeImage>[0]),
        };
